...
3. If `.cto/context/index.md` is missing, tell the user to run `/cto-init` or `/cto-continue [slug]` to create/check continuation context.
...
- If the slug is not indexed, say it is unknown and suggest `/cto-continue {slug}` to scaffold/check and continue it.
